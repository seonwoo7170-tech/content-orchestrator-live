...
var MAX_PUBLICATION_ATTEMPTS = 3;
var MIN_SCHEDULE_LEAD_MINUTES = 10;
var CARRYOVER_LOOKBACK_DAYS = 14;
var STALE_PUBLICATION_CLAIM_MINUTES = 15;
var PUBLISH_RATE_LIMIT_COOLDOWN_MINUTES = 30;
function requireDb17(env) {
  if (!env?.ORCHESTRATOR_DB) throw new Error("DB_NOT_BOUND");
  return env.ORCHESTRATOR_DB;
}
__name(requireDb17, "requireDb");
...
__name(timeInTimeZone, "timeInTimeZone");
function hhmm2(totalMinutes) {
...
function kstTimestamp(planDate, totalMinutes) {
  ...
  const base = /* @__PURE__ */ new Date(`${planDate}T00:00:00+09:00`);
  base.setUTCDate(base.getUTCDate() + dayOffset);
  const date = base.toLocaleDateString("en-CA", { timeZone: "Asia/Seoul" });
  return `${date}T${hhmm2(totalMinutes)}:00+09:00`;
}
__name(kstTimestamp, "kstTimestamp");
function addMinutes(timestamp, minutes) {
  const date = new Date(timestamp);
  if (!Number.isFinite(date.getTime())) throw new Error("PUBLICATION_SCHEDULE_TIMESTAMP_INVALID");
  return new Date(date.getTime() + Number(minutes) * 6e4).toISOString();
}
__name(addMinutes, "addMinutes");
function isPublicationDue(settings, slotNo) {
...
__name(isPublicationDue, "isPublicationDue");
...
__name(imageEvidence, "imageEvidence");
var PUBLISH_IMAGE_COUNT_EXCEPTION_JOB_IDS = /* @__PURE__ */ new Set([137, 139, 149, 151, 153, 154, 156, 157]);
var PUBLISH_IMAGE_COUNT_EXCEPTION_MIN_IMAGES = 3;
async function recordPublishBlock(env, jobId, reason) {
  const message = `\uBC1C\uD589 \uBCF4\uB958 \xB7 ${reason}`;
  try {
    const existing = await requireDb17(env).prepare(
...
__name(recordPublishBlock, "recordPublishBlock");
function validatePublicationResult(result, settings, images = [], jobId = null) {
  ...
  if (settings?.imagesEnabled && !imagesEvidence.passed) {
    const exempt = jobId !== null && PUBLISH_IMAGE_COUNT_EXCEPTION_JOB_IDS.has(Number(jobId)) && Number(imagesEvidence.attached || 0) >= PUBLISH_IMAGE_COUNT_EXCEPTION_MIN_IMAGES;
    if (!exempt) {
...
__name(validatePublicationResult, "validatePublicationResult");
async function listCandidates(env, planDate) {
  const result = await requireDb17(env).prepare(
...
async function previousScheduledPublication(env, candidate) {
  ...
  return requireDb17(env).prepare(
...
__name(previousScheduledPublication, "previousScheduledPublication");
...
__name(withInternalLinks, "withInternalLinks");
async function resolveScheduledAt(env, candidate, settings, now) {
  ...
  if (previous?.scheduled_time) {
    const gap = Number(settings.publishIntervalMinutes || 30) + deterministicPublishJitter(candidate.plan_date, candidate.blog_id, `gap:${candidate.slot_no}`, settings.publishJitterMinutes || 0);
    scheduledAt = addMinutes(previous.scheduled_time, gap);
  }
  const minimum = new Date(now.getTime() + MIN_SCHEDULE_LEAD_MINUTES * 6e4);
...
__name(resolveScheduledAt, "resolveScheduledAt");
async function holdStalePublicationClaims(env, now) {
  const db = requireDb17(env);
  const cutoff = new Date(now.getTime() - STALE_PUBLICATION_CLAIM_MINUTES * 6e4).toISOString();
...
__name(holdStalePublicationClaims, "holdStalePublicationClaims");
async function claimPublication(env, candidate, scheduledTime2) {
  const db = requireDb17(env);
  ...
  if (existing?.status === "failed") {
    ...
    ).bind(String(scheduledTime2), Number(candidate.job_id), MAX_PUBLICATION_ATTEMPTS).run();
...
__name(claimPublication, "claimPublication");
...
__name(markScheduled, "markScheduled");
...
__name(markVerificationPending, "markVerificationPending");
...
function isPublishRateLimitError(error) {
...
__name(isPublishRateLimitError, "isPublishRateLimitError");
...
__name(markAmbiguousWriteHeld, "markAmbiguousWriteHeld");
...
__name(isAmbiguousWriteError, "isAmbiguousWriteError");
function safeResult2(value) {
  try {
...
async function prepareInternalLinks(env, candidate, result, fetchImpl) {
  ...
  const nextResult = {
    ...
    strategy: {
      ...
      internalLinksAppliedAt: (/* @__PURE__ */ new Date()).toISOString()
    }
  };
  await persistJobResult(env, candidate.job_id, nextResult);
  return nextResult;
}
__name(prepareInternalLinks, "prepareInternalLinks");
...
__name(resultWithQualityAndEvidence, "resultWithQualityAndEvidence");
async function runDueAutoPublications(env, blogs, options = {}) {
  ...
  const settingsByBlog3 = new Map((automation.blogs || []).map((item) => [String(item.blogId), item.effective]));
  ...
  for (const candidate of candidates) {
    const settings = settingsByBlog3.get(String(candidate.blog_id));
    if (!settings || !isPublicationDue(settings, candidate.slot_no)) continue;
    let result = safeResult2(candidate.result_json);
    ...
    try {
      ...
      const readbackCheck = readback ? validateBloggerReadback({ blogId: String(candidate.blog_id), bloggerPostId: String(published.bloggerPostId) }, readback) : { passed: false };
      ...
      if (isPublishRateLimitError(error)) {
        ...
        try {
          await persistJobTransition(env, candidate.job_id, "failed", { error: error?.message || "BLOGGER_API_429" });
        } catch {
        }
        ...
        try {
          await persistJobTransition(env, candidate.job_id, "failed", { error: "BLOGGER_WRITE_OUTCOME_UNKNOWN" });
        } catch {
        }
        ...
        try {
          await persistJobTransition(env, candidate.job_id, "failed", { error: error?.message || "AUTO_PUBLISH_FAILED" });
        } catch {
        }
...
__name(runDueAutoPublications, "runDueAutoPublications");
