...
PUBLICATION METADATA — never write a publication date, an update date, a review date or a byline into html. Blogger renders the real publication date from the post record itself, so a date in the body produces two dates on one page that disagree, and the one you write is invented: you do not know when the post will be published. Do not emit "Published:", "Last updated:", "Updated:", "Reviewed:", 게시일, 작성일 or 최종 수정 anywhere in html, and do not put a date in a byline block. The server strips such a line before publication, so writing one only costs you body content. A date belongs in the body only when it is part of the subject matter -- the effective date of a regulation, the model year of a product, the date a cited document was issued.
