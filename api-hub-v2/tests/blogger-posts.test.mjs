  ...
  const fetchImpl = mockGoogle(async (url) => {
    ...
    if (!parsed.searchParams.get('pageToken')) {
      return new Response(JSON.stringify({
        items: [
          { id: '2', title: 'Newer', url: 'https://example.com/newer', published: '2026-02-02T00:00:00Z', labels: ['b'] },
          { id: '1', title: 'Older', url: 'https://example.com/older', published: '2026-01-01T00:00:00Z', labels: ['a'] }
        ],
        nextPageToken: 'next'
      }), { status: 200 });
    }
    ...
    return new Response(JSON.stringify({
      items: [{ id: '3', title: 'Newest', url: 'https://example.com/newest', published: '2026-03-03T00:00:00Z' }]
    }), { status: 200 });
  });
